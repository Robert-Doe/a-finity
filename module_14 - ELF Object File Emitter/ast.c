...
void node_print(const Node *n, FILE *fp, int indent) {
    ...
    switch (n->kind) {
        ...
        case AST_IDENT: case AST_VAR_DECL: case AST_FUNC: case AST_CALL:
            if (n->sval) fprintf(fp, " \"%s\"", n->sval); break;
        case AST_UNARY: case AST_BINARY:
            fprintf(fp, " '%c'", n->op); break;
        default: break;
    }
    if (n->left || n->right || n->extra || n->nargs > 0) fprintf(fp, "\n");
    ...
    if (n->left || n->right || n->extra || n->nargs > 0) indent_print(fp, indent);
    fprintf(fp, ")\n");
}
