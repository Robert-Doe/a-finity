/* sema.c — Semantic analysis for my_compiler */
#include "sema.h"
...
static void check_expr(SemaCtx *ctx, const Node *n) {
    ...
    switch (n->kind) {
        ...
        case AST_IDENT:
            ...
            break;
        case AST_UNARY: check_expr(ctx, n->left); break;
        case AST_BINARY:
        ...
        case AST_CALL: {
            ...
            if (!sym) err(ctx, n->line, "undefined function", n->sval);
            else if (sym->n_params != n->nargs) {
                fprintf(stderr, "sema error at line %d: function '%s' expects %d args, got %d\n",
...
static void check_stmt(SemaCtx *ctx, const Node *n) {
    ...
    switch (n->kind) {
        case AST_VAR_DECL:
            ...
            break;
        case AST_RETURN: check_expr(ctx, n->left); break;
        case AST_IF:
            check_expr(ctx, n->left);
        ...
        case AST_BLOCK:
            ...
            break;
        case AST_EXPR_STMT: check_expr(ctx, n->left); break;
        default: break;
    }
}
