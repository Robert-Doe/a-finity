...
#define MAX_TEMPS 512 /* maximum number of temporaries in one function */

/* known[t] = 1  means we know the value of temporary t.
 * val[t]        is the known value when known[t] == 1. */
...
static void fold_constants(IRFunc *f) {
    ...
    for (IRInstr *in = f->head; in; in = in->next) {
        ...
        /* A constant load: record the known value. */
        case IR_ICONST:
        ...
        /* Unary negation: fold if operand is known. */
        case IR_NEG:
        ...
        /* Binary arithmetic / comparison: fold if both operands are known. */
...
/* copy_src[t] = s means "t was defined by COPY s; replace uses of t with s" */
...
static void propagate_copies(IRFunc *f) {
    ...
    for (IRInstr *in = f->head; in; in = in->next) {
        ...
        if (in->op == IR_COPY && in->dst >= 0 && in->dst < MAX_TEMPS) {
            /* Record this as a copy so downstream uses get the original */
            int src = in->src1;
            /* Chase chains: if src itself was a copy, follow it */
            while (src >= 0 && src < MAX_TEMPS && copy_src[src] >= 0)
                src = copy_src[src];
            copy_src[in->dst] = src;
        } else if (in->dst >= 0 && in->dst < MAX_TEMPS) {
            /* This temp is redefined; its copy mapping is no longer valid */
            copy_src[in->dst] = -1;
        }
    }
}
