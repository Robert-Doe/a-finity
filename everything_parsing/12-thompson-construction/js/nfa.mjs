...
export class Nfa {
  ...
  static parse(text) {
    ...
    if (!start || states.length === 0)
      throw new Error("NFA needs states and a start state");
    // (an empty alphabet is legal: the NFA then accepts at most the empty string)
...
function rest(line) { const r = line.slice(line.indexOf(":") + 1).trim(); return r === "" ? [] : r.split(/\s+/); }
