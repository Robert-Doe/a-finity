 ...
 * Module 15: Standard Library Shim (adds IR_PRINT emission for AST_PRINT)
 */
#include "ir.h"
#include "token.h"
...
static int gen_expr(IRFunc *f, Node *n) {
    ...
    case AST_BINARY: {
        ...
        switch (n->op) {
            ...
            case TOK_AMPAMP:   op = IR_AND; break;
            case TOK_PIPEPIPE: op = IR_OR;  break;
            default:
...
static void gen_stmt(IRFunc *f, Node *n) {
    ...
    /* Module 15: print(expr) — evaluate the expression into a temp,
     * then emit IR_PRINT with src1 = that temp and dst = IR_NO_TEMP. */
    case AST_PRINT: {
        int val = gen_expr(f, n->left);
        IRInstr *pr = ir_new_instr(IR_PRINT);
        pr->src1 = val;
        pr->dst  = IR_NO_TEMP;
        ir_append(f, pr);
        break;
