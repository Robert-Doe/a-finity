 ...
 * Module 15: Standard Library Shim (adds parse_stmt case for TOK_KW_PRINT)
 */
#include "parser.h"
...
static Token expect(Parser *p, TokenType type, const char *what) {
    if (p->lx->current.type != type) {
        fprintf(stderr, "line %d: expected %s (got %s)\n",
                p->lx->current.line, what, token_type_name(p->lx->current.type));
        p->had_error = 1;
    }
    return advance(p);
}
...
static Node *parse_expr(Parser *p) {
    ...
    if (peek(p).type == TOK_IDENT) {
        ...
        /* Not an assignment — rebuild the left side */
        Node *ident;
        if (check(p, TOK_LPAREN)) {
            advance(p);
        ...
        /* Continue binary parsing with ident as left */
        for (;;) {
            int prec = binary_op_prec(peek(p).type);
            if (prec < 1) break;
            Token op = advance(p);
...
static Node *parse_stmt(Parser *p) {
    ...
    if (t.type == TOK_KW_PRINT) {
        ...
        expect(p, TOK_RPAREN, "')'");
        expect(p, TOK_SEMICOLON, "';'");
        Node *n = node_new(AST_PRINT, t.line);
        n->left = expr;
        return n;
    }
...
/* Parse: int|void name ( params ) { body } */
...
/* Suppress unused warning for match() */
static int match_unused_suppress(Parser *p, TokenType type) {
    return match(p, type);
}
static void suppress_warnings(void) __attribute__((unused));
static void suppress_warnings(void) { (void)match_unused_suppress; }
