 ...
 * Module 15: Standard Library Shim (adds IR_PRINT)
...
typedef enum {
    ...
    IR_AND,     /* dst = src1 && src2                       logical and */
    ...
    IR_PARAM,   /* push argument: src1                      pass one argument to next call */
    IR_CALL,    /* dst = call name, nargs args              call a function */
    IR_RETURN,  /* return src1  (IR_NO_TEMP = void return)  return from function */
    IR_STORE,   /* memory[name] = src1                      write a local variable */
    IR_LOAD,    /* dst = memory[name]                       read a local variable */
    IR_PRINT,   /* print_int(src1)  dst = IR_NO_TEMP        Module 15: emit output */
} IROp;
...
typedef struct {
    ...
    int      next_label; /* counter: next label number */
...
/* Free all memory allocated by irgen. */
