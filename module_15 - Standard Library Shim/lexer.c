/* lexer.c — Lexer implementation for my_compiler
 * Module 15: Standard Library Shim (adds "print" keyword)
 */
#include "lexer.h"

#include <ctype.h>
#include <string.h>
#include <stdlib.h>

/* ------------------------------------------------------------------ */
/* Internal helpers                                                      */
/* ------------------------------------------------------------------ */

/* Return the current character without advancing. */
static char peek_char(Lexer *lx) {
    if (lx->src->pos >= lx->src->len) return '\0';
    return lx->src->text[lx->src->pos];
}

/* Advance one character and return it. Updates line/col tracking. */
static char advance_char(Lexer *lx) {
    char c = lx->src->text[lx->src->pos++];
    if (c == '\n') { lx->src->line++; lx->src->col = 1; }
    else           { lx->src->col++; }
    return c;
}

/* Skip whitespace and comments (// line and /* block */). */
static void skip_whitespace(Lexer *lx) {
    for (;;) {
        while (isspace((unsigned char)peek_char(lx))) advance_char(lx);
        /* Line comment */
        if (peek_char(lx) == '/' && lx->src->pos + 1 < lx->src->len &&
            lx->src->text[lx->src->pos + 1] == '/') {
            while (peek_char(lx) != '\n' && peek_char(lx) != '\0') advance_char(lx);
            continue;
        }
        /* Block comment */
        if (peek_char(lx) == '/' && lx->src->pos + 1 < lx->src->len &&
            lx->src->text[lx->src->pos + 1] == '*') {
            advance_char(lx); advance_char(lx); /* consume / * */
            while (!(peek_char(lx) == '*' && lx->src->pos + 1 < lx->src->len &&
                     lx->src->text[lx->src->pos + 1] == '/') && peek_char(lx) != '\0') {
                advance_char(lx);
            }
            if (peek_char(lx) != '\0') { advance_char(lx); advance_char(lx); } /* consume * / */
            continue;
        }
        break;
    }
}

/* Check if text is a keyword; return its token type or TOK_IDENT. */
static TokenType keyword_or_ident(const char *text) {
    if (strcmp(text, "int")    == 0) return TOK_KW_INT;
    if (strcmp(text, "void")   == 0) return TOK_KW_VOID;
    if (strcmp(text, "return") == 0) return TOK_KW_RETURN;
    if (strcmp(text, "if")     == 0) return TOK_KW_IF;
    if (strcmp(text, "else")   == 0) return TOK_KW_ELSE;
    if (strcmp(text, "while")  == 0) return TOK_KW_WHILE;
    if (strcmp(text, "print")  == 0) return TOK_KW_PRINT;  /* Module 15 */
    return TOK_IDENT;
}

/* ------------------------------------------------------------------ */
/* Core lexer function                                                   */
/* ------------------------------------------------------------------ */

/* Lex and return the next token from src. */
static Token lex_one(Lexer *lx) {
    skip_whitespace(lx);

    Token tok;
    memset(&tok, 0, sizeof(tok));
    tok.line = lx->src->line;
    tok.col  = lx->src->col;

    char c = peek_char(lx);
    if (c == '\0') { tok.type = TOK_EOF; return tok; }

    /* Integer literal */
    if (isdigit((unsigned char)c)) {
        long val = 0;
        while (isdigit((unsigned char)peek_char(lx))) {
            val = val * 10 + (advance_char(lx) - '0');
        }
        tok.type = TOK_INT_LIT;
        tok.ival = val;
        return tok;
    }

    /* Identifier or keyword */
    if (isalpha((unsigned char)c) || c == '_') {
        int i = 0;
        while ((isalnum((unsigned char)peek_char(lx)) || peek_char(lx) == '_') && i < 63) {
            tok.text[i++] = advance_char(lx);
        }
        tok.text[i] = '\0';
        tok.type = keyword_or_ident(tok.text);
        return tok;
    }

    advance_char(lx); /* consume the character */

    switch (c) {
        case '+': tok.type = TOK_PLUS;      break;
        case '-': tok.type = TOK_MINUS;     break;
        case '*': tok.type = TOK_STAR;      break;
        case '/': tok.type = TOK_SLASH;     break;
        case '%': tok.type = TOK_PERCENT;   break;
        case '(': tok.type = TOK_LPAREN;    break;
        case ')': tok.type = TOK_RPAREN;    break;
        case '{': tok.type = TOK_LBRACE;    break;
        case '}': tok.type = TOK_RBRACE;    break;
        case ';': tok.type = TOK_SEMICOLON; break;
        case ',': tok.type = TOK_COMMA;     break;
        case '<':
            if (peek_char(lx) == '=') { advance_char(lx); tok.type = TOK_LEQ; }
            else                       tok.type = TOK_LT;
            break;
        case '>':
            if (peek_char(lx) == '=') { advance_char(lx); tok.type = TOK_GEQ; }
            else                       tok.type = TOK_GT;
            break;
        case '=':
            if (peek_char(lx) == '=') { advance_char(lx); tok.type = TOK_EQ; }
            else                       tok.type = TOK_ASSIGN;
            break;
        case '!':
            if (peek_char(lx) == '=') { advance_char(lx); tok.type = TOK_NEQ; }
            else                       tok.type = TOK_BANG;
            break;
        case '&':
            if (peek_char(lx) == '&') { advance_char(lx); tok.type = TOK_AMPAMP; }
            else                       tok.type = TOK_ERROR;
            break;
        case '|':
            if (peek_char(lx) == '|') { advance_char(lx); tok.type = TOK_PIPEPIPE; }
            else                       tok.type = TOK_ERROR;
            break;
        default:
            tok.type = TOK_ERROR;
            break;
    }
    return tok;
}

/* ------------------------------------------------------------------ */
/* Public API                                                            */
/* ------------------------------------------------------------------ */

void lexer_init(Lexer *lx, Source *src) {
    lx->src = src;
    lx->current = lex_one(lx); /* prime the lookahead */
}

Token lexer_next(Lexer *lx) {
    Token t = lx->current;
    lx->current = lex_one(lx);
    return t;
}
