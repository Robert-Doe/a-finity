...
/* suppress unused-function warning for match() — it is part of the
 * standard parser toolkit even if not every grammar rule needs it */
static int match_suppress_unused(Parser *p, TokenType type) {
    return match(p, type);
}
/* make the compiler believe it is used */
static void (*match_ref)(Parser *, TokenType) =
    (void (*)(Parser *, TokenType))match_suppress_unused;
...
Node *parse_program(Parser *p, Lexer *lx) {
    ...
    /* Suppress unused function pointer warning */
    (void)match_ref;
